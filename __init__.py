...
class FlowZwerg(Application):
    ...
    def draw(self, ctx: Context) -> None:
        ...
        ctx.save()
        ctx.move_to(0, -75).rgb(255, 255, 255).text('GnomeData')
        ctx.font_size = 24
        ctx.move_to(0, -45).rgb(255, 255, 255).text(self.gnomes[self.current_gnome])
        ctx.font_size = 16
        ctx.save()
        ...
        try:
            ...
            for row in rows:
                ...
                if values[4] is 'temperature':
                    self.temperature = round(float(values[3]), 2)
                elif values[4] is 'humidity':
                    self.humidity = round(float(values[3]), 2)
                elif values[4] is 'pressure':
                    self.air_preassure = round(float(values[3]), 2)
                elif values[4] is 'uv_index':
                    self.uv_index = int(values[3])
                elif values[4] is 'sound_pressure':
                    self.sound_pressure = round(float(values[3]), 2)
        except:
